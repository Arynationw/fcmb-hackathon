...
const ContextProvider = ({children}) => {
    ...
    const getUser = async ()=> {
      try {
        ...
        cookies.set(import.meta.env.VITE_APP_NAME,btoa(data.name),{
        ...
        setLoadingUser(false)
         if (error?.response?.status === 401)
            { 
              if (cookies.get(import.meta.env.VITE_APP_NAME)) {
                 cookies.remove(import.meta.env.VITE_APP_NAME)
               }
               setUser(null)
            }
  ...
  return (
    <stateContext.Provider value={{surveys,getUser,user,setUser}}>
    {children}
